...
#
# -v - odwraca dopasowanie, -E - używa wyrażeń regularnych (| do OR), -d' ' - dzieli plik po spacji, -f1 - wybiera pierwsze pole (nazwę użytkownika)
cat dodatkowe/etc-passwd | grep -vE '(/bin/nologin|/bin/false)$' | cut -d' ' -f1